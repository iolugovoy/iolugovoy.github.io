...
//     data_src: 'src'
// });
_bindEvents();this.currentScale=_getNewScale();$(window).resize(function(){var newScale=_getNewScale();if(newScale!==_this.currentScale){_this.currentScale=newScale;_this.emit('changeScale',newScale);}else{_this.emit('changeViewport');}_this.emit('resize',{scale:_this.currentScale});});var resizeStop=debounce(function(data){_this.emit('resizeStop',data);},500);this.on('resize',resizeStop);// svg4everybody();
if($('include[src]:empty').length){var interval=setInterval(function(){if($('include[src]:empty').lenght){return false;}clearInterval(interval);_this.emitInit();},100);}else{this.emitInit();}},scrollLock:function scrollLock(){_scroll=scrollTop();this.emit('scrollLock');$('.body').css({top:-_scroll});$('html').addClass('scroll-locked');$('body').css('padding-right',$('body').hasScrollBar()?getScrollbarWidth():'');$('[data-id="scroll-lock"]').css('right',$('body').hasScrollBar()?getScrollbarWidth():'');_scrollLocked=true;},scrollUnlock:function scrollUnlock(){if(_scrollLocked){$('body').css('padding-right','');$('.body').css({top:''});$('[data-id="scroll-lock"]').css('right','');$('html').removeClass('scroll-locked');scrollTop(_scroll);_scrollLocked=false;this.emit('scrollUnlock');}}},new Events());}(jQuery);$(function(){app.init();});;function getImageSize(src){var newImg=new Image();return new Promise(function(resolve){newImg.onload=function(){resolve({width:newImg.width,height:newImg.height});};newImg.src=src;});}function debounce(func,wait,immediate){var timeout=void 0;return function(){var context=this,args=arguments;var later=function later(){timeout=null;if(!immediate)func.apply(context,args);};var callNow=immediate&&!timeout;clearTimeout(timeout);timeout=setTimeout(later,wait);if(callNow)func.apply(context,args);};}function isInScales(scaleNames){return $.inArray(app.currentScale.name,scaleNames)!==-1;}var CallbackRegistry={};// реестр
